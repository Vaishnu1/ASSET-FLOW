...
@Component({
  selector: 'app-rtv-grn-list',
  templateUrl: './rtv-grn-list.component.html',
  styleUrls: ['./rtv-grn-list.component.css']
})
export class RtvGrnListComponent implements OnInit {

  displayedColumns = ['select','grnNo','doNo','item','itemDesc','grnDate', 'supplierName','supplierSiteName','createdBy'];
  grnDataSource = [];
  ...
  selectedItem : any = 0;
  constructor(public dialogRef: MatDialogRef<RtvGrnListComponent>,
    ...
    fetchListOfGRN(){
      ...
      this.commonService.commonListService('fetchListOfAllGrn.sams','').subscribe(
        data => {
          if(data.success){
