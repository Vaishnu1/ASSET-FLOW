...
@Component({
  selector: 'app-sr-pr-report',
  templateUrl: './sr-pr-report.component.html',
  styleUrls: ['./sr-pr-report.component.css']
})
export class SrPrReportComponent implements OnInit {
  ...
  itemCombo: any = [];
  loadItemComboData(searchValue) {
    this.commonService.getComboResults(this.samsService.listAllItemCombo, searchValue.term, '', '', 0, 25).subscribe(
      (data) => {
  ...
  selectedItemData(event) {
    this.srReportForm.controls.manufacturerId.setValue(event.itemId);
    this.srReportForm.controls.manufacturerName.setValue(event.itemName);
  }
