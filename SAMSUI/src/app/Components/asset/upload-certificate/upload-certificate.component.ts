...
export class UploadCertificateComponent implements OnInit {
  ...
  constructor(private commonService: CommonService,
              public dialogRef: MatDialogRef<UploadCertificateComponent>,
  ...
  uploadAssetDocFile() {
    ...
    var certificateData={
                    ...
                    'issuingAuthority': this.uploadDocForCertificate.controls.issuingAuthority.value,
                    'orgId':this.userSessionService.getUserOrgId(),
                    'userId':this.userSessionService.getUserId(),
                    'assetHdrId': this.uploadDocForCertificate.controls.assetHdrId.value,
                    'docName': this.fileName
                  };
    formData.append('certificateImageData',JSON.stringify(certificateData));
    //To start loading
    this.commonService.commonFileUpload('saveOrUpdateAssetCertificate.sams', formData).subscribe(
      data => {
        if (data.success) {
