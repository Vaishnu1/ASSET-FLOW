...
@Component({
  selector: 'app-service-request-list-pm',
  templateUrl: './service-request-list-pm.component.html',
  styleUrls: ['./service-request-list-pm.component.css']   
})
export class ServiceRequestListPMComponent implements OnInit,OnDestroy {
  //set page title
  title = 'Asset Optima - Preventive Maintenance Service Request';
  ...
  ngOnInit() {
    ...
    this.fetchList();
    //this.moduleAccessModel=this.userSessionService.getUserGroupAccess()['PREVENTIVE MAINTENANCE'];
  }
  ...
  createService(srType){
    this.dialogRef = this.dialog.open(ServiceRequestCreateComponent, {
      height: '600px',
      width: '767px',
      data: {
        'srType':srType
  ...
  fetchList(){
    ...
    this.serviceRequestModel.srType='PM';
    this.dataSource = [];
    this.subloader = true;
